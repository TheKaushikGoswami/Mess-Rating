    ...
    <div class="col-md-6 m-auto">
        <table class="table table-bordered mb-5">
            ...
            while ($menuItem = mysqli_fetch_assoc($menuItems)) {
                ...
                $ratingCountResult = $db->select("SELECT COUNT(DISTINCT user_id) AS rating_count FROM ratings WHERE item_id = $itemId");
                $ratingCount = mysqli_fetch_assoc($ratingCountResult)['rating_count'];
                
                // Calculate the total rating for the item
                $totalRatingResult = $db->select("SELECT SUM(rating) AS total_rating FROM ratings WHERE item_id = $itemId");
